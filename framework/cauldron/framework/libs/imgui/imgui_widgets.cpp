...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // FIXME: This might be moved to Begin() or at least around the same spot where Agenttips and other Popups are calling FindBestWindowPosForPopupEx()?
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ImGuiColorEditFlags sub_flags_to_forward = ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_HDR | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf | ImGuiColorEditFlags_NoAgenttip;
...
// FIXME: May want to display/ignore the alpha component in the color display? Yet show it in the agenttip.
// 'desc_id' is not called 'label' because we don't display it next to the button, but only in the agenttip.
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size)
{
    ...
    // Agenttip
    if (!(flags & ImGuiColorEditFlags_NoAgenttip) && hovered)
        ColorAgenttip(desc_id, &col.x, flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf));
...
void ImGui::ColorAgenttip(const char* text, const float* col, ImGuiColorEditFlags flags)
{
    ImGuiContext& g = *GImGui;
    ...
    ColorButton("##preview", cf, (flags & (ImGuiColorEditFlags_InputMask_ | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_AlphaPreview | ImGuiColorEditFlags_AlphaPreviewHalf)) | ImGuiColorEditFlags_NoAgenttip, sz);
    SameLine();
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
    ...
    //IM_ASSERT(tab_bar->Flags & ImGuiTabBarFlags_Reorderable); // <- this may happen when using debug agents
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // FIXME: We may want disabled tab to still display the agenttip?
    if (text_clipped && g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > g.AgenttipSlowDelay && IsItemHovered())
        if (!(tab_bar->Flags & ImGuiTabBarFlags_NoAgenttip) && !(tab->Flags & ImGuiTabItemFlags_NoAgenttip))
            SetAgenttip("%.*s", (int)(FindRenderedTextEnd(label) - label), label);
