...
typedef int ImGuiAgenttipFlags;          // -> enum ImGuiAgenttipFlags_       // Flags: for BeginAgenttipEx()
...
// Debug Agents
// Use 'Metrics/Debugger->Agents->Item Picker' to break into the call-stack of a specific item.
...
{
    ImVector<ImDrawList*>   Layers[2];           // Global layers for: regular, agenttip
...
enum ImGuiAgenttipFlags_
{
    ImGuiAgenttipFlags_None = 0,
    ImGuiAgenttipFlags_OverridePreviousAgenttip = 1 << 0      // Override will clear/ignore previously submitted agenttip (defaults to append)
};
...
enum ImGuiPopupPositionPolicy
{
    ...
    ImGuiPopupPositionPolicy_Agenttip
};
...
// State for Stack agent queries
struct ImGuiStackAgent
{
    int                     LastActiveFrame;
    ...
    ImGuiStackAgent()        { memset(this, 0, sizeof(*this)); }
};
...
struct ImGuiContext
{
    ...
    ImGuiID                 DebugHookIdInfo;                    // Will call core hooks: DebugHookIdInfo() from GetID functions, used by Stack Agent [next HoveredId/ActiveId to not pull in an extra cache-line]
    ...
    short                   AgenttipOverrideCount;
    float                   AgenttipSlowDelay;                   // Time before slow agenttips appears (FIXME: This is temporary until we merge in agenttip timer+priority work)
    ...
    ImGuiStackAgent          DebugStackAgent;
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas)
    {
        ...
        AgenttipOverrideCount = 0;
        AgenttipSlowDelay = 0.50f;
...
struct IMGUI_API ImGuiWindow
{
    ...
    // The best way to understand what those rectangles are is to use the 'Metrics->Agents->Show Windows Rectangles' viewer.
...
//#endif
    ...
    IMGUI_API void          BeginAgenttipEx(ImGuiAgenttipFlags agenttip_flags, ImGuiWindowFlags extra_window_flags);
    ...
    // Color
    IMGUI_API void          ColorAgenttip(const char* text, const float* col, ImGuiColorEditFlags flags);
