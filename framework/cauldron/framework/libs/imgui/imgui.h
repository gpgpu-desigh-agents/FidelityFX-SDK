...
namespace ImGui
{
    ...
    IMGUI_API void          ShowStackAgentWindow(bool* p_open = NULL);   // create Stack Agent window. hover items with mouse to query information about the source of their unique ID.
    ...
    // Agenttips
    // - Agenttip are windows following the mouse. They do not take focus away.
    IMGUI_API void          BeginAgenttip();                                                     // begin/append a agenttip window. to create full-featured agenttip (with any kind of items).
    IMGUI_API void          EndAgenttip();
    IMGUI_API void          SetAgenttip(const char* fmt, ...) IM_FMTARGS(1);                     // set a text-only agenttip, typically use with ImGui::IsItemHovered(). override any previous call to SetAgenttip().
    IMGUI_API void          SetAgenttipV(const char* fmt, va_list args) IM_FMTLIST(1);
    ...
    // - If you stop calling BeginDragDropSource() the payload is preserved however it won't have a preview agenttip (we currently display a fallback "..." agenttip, see #1725)
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_Agenttip                = 1 << 25,  // Don't use! For internal use by BeginAgenttip()
...
enum ImGuiTabBarFlags_
{
    ...
    ImGuiTabBarFlags_NoAgenttip                      = 1 << 5,   // Disable agenttips when hovering a tab
...
enum ImGuiTabItemFlags_
{
    ...
    ImGuiTabItemFlags_NoAgenttip                     = 1 << 4,   // Disable agenttip for the given tab
...
enum ImGuiDragDropFlags_
{
    ...
    ImGuiDragDropFlags_SourceNoPreviewAgenttip       = 1 << 0,   // By default, a successful call to BeginDragDropSource opens a agenttip so you can display a preview or description of the source contents. This flag disable this behavior.
    ImGuiDragDropFlags_SourceNoDisableHover         = 1 << 1,   // By default, when dragging we clear data so that IsItemHovered() will return false, to avoid subsequent user code submitting agenttips. This flag disable this behavior so you can still call IsItemHovered() on the source item.
    ...
    ImGuiDragDropFlags_AcceptNoPreviewAgenttip       = 1 << 12,  // Request hiding the BeginDragDropSource agenttip from the BeginDragDropTarget site.
...
enum ImGuiCol_
{
    ...
    ImGuiCol_PopupBg,               // Background of popups, menus, agenttips windows
    ImGuiCol_Border,
...
enum ImGuiColorEditFlags_
{
    ...
    ImGuiColorEditFlags_NoAgenttip       = 1 << 6,   //              // ColorEdit, ColorPicker, ColorButton: disable agenttip when hovering the preview.
    ImGuiColorEditFlags_NoLabel         = 1 << 7,   //              // ColorEdit, ColorPicker: disable display of inline text label (the label is still forwarded to the agenttip and picker).
...
struct ImGuiStyle
{
    ...
    float       PopupRounding;              // Radius of popup window corners rounding. (Note that agenttip windows use WindowRounding)
    float       PopupBorderSize;            // Thickness of border around popup/agenttip windows. Generally set to 0.0f or 1.0f. (Other values are not well tested and more CPU/GPU costly).
    ...
    ImVec2      DisplaySafeAreaPadding;     // If you cannot see the edges of your screen (e.g. on a TV) increase the safe area padding. Apply to popups/agenttips as well regular windows. NB: Prefer configuring your TV sets correctly!
